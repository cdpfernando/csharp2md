...
- **Phase / Task**: Phase 12 (close the Component/DeploymentUnit-scope fan-out), **T79 committed and complete**. This phase was opened by the fix→re-verify loop after the queued full-scope Verifier (dispatched per Phase 11's own next step) returned **FAIL**: 70 of 71 ACs + 5/5 edge cases matched their spec outcome, but DEP-01 failed at Component/DeploymentUnit scope — 138 of 144 possible pairs (95.8%) on `fixtures/eShop`, the exact residual this file had named and left open as "not measured as a live defect." Full report at `.specs/features/pacote-conhecimento-util-e-confiavel/validation.md` (supersedes the Phase-11-scoped PASS before it). **A second full-scope Verifier dispatch (iteration 2 of the bounded 3-iteration fix→re-verify loop) is the queued next step** — not yet run.
- **Completed**: T1-T67, T69-T79. Phase 10: `bd901e4` (T70), `1386f9e` (T71), `aed46f5` (T72). Phase 11: `997fb7a` (T73), `7545454` (T74), `c5d69c3` (T75), `926f5bb` (T76), `e76894d` (T77), `d826c96` (T78). Phase 12: T79 (commit pending in this session).
