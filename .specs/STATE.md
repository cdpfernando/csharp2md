...
- **Next step — user decision, 2026-09-17**: of the two open strategic options, the user chose **re-verify the whole feature end to end**, not narrow the product scope for now. That means: dispatch a fresh, full-scope Verifier (author ≠ verifier) against **all 71 acceptance criteria** in `spec.md` (PKG-01..10, DEP-01..08, MET-01..08, NAV-01..10, VAR-01..06, STO-01..07, PUB-01..08, CRT-01..09, EDG-01..05) — not diff-scoped to Phase 11 like the last run, and not leaning on `fixtures/SyntheticSolution` alone the way the pre-Phase-11 PASS did (it cannot discriminate a correct dependency graph from a complete one — that is exactly what Phase 11 found and fixed). Anchor evidence to the real, discriminating instruments now available: `fixtures/ArchitectureDependencyLab` (the 87-edge oracle) for DEP-*, and `fixtures/CertificationCorpus`/`fixtures/PublicationResilience` where they apply, alongside the existing unit/integration suites. Follow `references/validate.md`'s full process (spec-anchored coverage check evidence-or-zero, confirm each test's asserted value against the spec-defined outcome — not just "some assertion exists" — discrimination sensor **skipped** per the standing `AGENTS.md` rule), write a fresh `validation.md` (this supersedes the Phase-11-scoped one just written), and route any ranked gaps through the bounded fix→re-verify loop (max 3 iterations) before escalating to the user. **Not yet started** — this is the queued instruction for the next session/dispatch, recorded per the user's explicit choice in chat, not carried out yet. The Verifier's 5 non-blocking observations from the Phase 11 pass (naming/trait/doc nits, listed below) are not coverage gaps and can be picked up opportunistically during this pass or separately.
