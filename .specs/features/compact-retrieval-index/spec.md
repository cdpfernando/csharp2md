...
| CRI-04 | P1: Recuperar relações por postings compactos | T9 | `Schema1RetrievalIndexProjectorTests.AllFiveLookups_DeepEqualFrozenSchemaOneLogicalPayloads` compares every relation/evidence field for all five families against the frozen schema-1 oracle. | Implemented; pending Verifier |
...
| CRI-07 | P1: Escrever shards limitados com custo linear | T5 | `RetrievalIndexProjectorTests.Project_DefaultWriterAccepts262144AndRejects262145Byte*Shard` measures production relation, metadata and posting shards; `Project_DelimiterClosingAndLfOverflowStartsAnotherProductionShard` proves rollover. | Implemented; pending Verifier |
| CRI-08 | P1: Escrever shards limitados com custo linear | T5 | The three production-boundary tests assert exact 262144-byte descriptors for relation, metadata and posting artifacts. | Implemented; pending Verifier |
| CRI-09 | P1: Escrever shards limitados com custo linear | T5 | The three production-boundary tests assert deterministic 262145-byte failures with the exact record identity/key and 262144-byte ceiling. | Implemented; pending Verifier |
...
| CRI-12 | P1: Escrever shards limitados com custo linear | T11 | `RetrievalIndexBenchmarkHarnessTests.SchemaTwo_TenAndTenThousandUniqueKeysCreateTheSameDirectorySet` projects both cardinalities to disk and asserts the exact flat directory set. | Implemented; pending Verifier |
...
| CRI-15 | P1: Publicar um contrato compacto inequívoco e determinístico | T4 | `RetrievalIndexProjectorTests.Project_NormalizesMetadataKnownFieldsExtensionsAndUnknowns` asserts schema 2 on relation, metadata, summary and UNKNOWN artifacts in addition to existing manifest/posting coverage. | Implemented; pending Verifier |
| CRI-16 | P1: Publicar um contrato compacto inequívoco e determinístico | T9 | `RetrievalIndexReaderV2Tests.Reader_RejectsUnknownSchemaInEveryResolvedArtifactKind` mutates posting, relation, metadata and UNKNOWN artifacts; `Open_UsesAnalysisTrustAndRestoreOnlyFromManifest` proves manifest ownership. | Implemented; pending Verifier |
...
| CRI-22 | P1: Manter UNKNOWNs em um único artefato | T8 | `RetrievalIndexProjectorTests.Project_NormalizesMetadataKnownFieldsExtensionsAndUnknowns` asserts the exact summary count/path shape and absence of UNKNOWN entries. | Implemented; pending Verifier |
| CRI-23 | P1: Manter UNKNOWNs em um único artefato | T9 | `RetrievalIndexReaderV2Tests.ReadUnknownGroups_ReconstructsCompleteRelations` asserts the complete logical UNKNOWN group, including `has_proven_entry_point: false` and `impact: 1`. | Implemented; pending Verifier |
...
| CRI-25 | P1: Eliminar metadados e projeções redundantes | T7 | `RetrievalIndexProjectorTests.Project_NormalizesMetadataKnownFieldsExtensionsAndUnknowns` projects two relations sharing one origin and asserts one physical origin plus provenance-field omission from each evidence record. | Implemented; pending Verifier |
...
| CRI-27 | P1: Eliminar metadados e projeções redundantes | T8 | The normalized projector test asserts the summary omits analysis/trust/restore; `RetrievalIndexReaderV2Tests.Open_UsesAnalysisTrustAndRestoreOnlyFromManifest` reconstructs them without opening the summary. | Implemented; pending Verifier |
...
| CRI-29 | P1: Eliminar metadados e projeções redundantes | T7 | Builder metric tests plus `RetrievalIndexProjectorTests.Project_NormalizesMetadataKnownFieldsExtensionsAndUnknowns` assert the exact serialized metric keys and absence of percentage fields. | Implemented; pending Verifier |
...
| CRI-31 | P1: Eliminar metadados e projeções redundantes | T9 | `Schema1RetrievalIndexProjectorTests.AllFiveLookups_DeepEqualFrozenSchemaOneLogicalPayloads` and `RelationRetrievalIndexEndToEndTests.MultipleEvidenceDocuments_ReconstructEveryFieldThroughPostingLookup` assert complete reconstruction against the frozen oracle and for multiple evidence records. | Implemented; pending Verifier |
...
| CRI-35 | P1: Comprovar a redução em corpus grande | T12 | `RetrievalIndexBenchmarkHarnessTests.VerifyCommand_MissingEqualOrRegressedMetricsExitNonZeroAndNameEveryMetric` executes the benchmark process for missing, equal and regressed reports and asserts non-zero exits plus all failed metric names. | Implemented; pending Verifier |
