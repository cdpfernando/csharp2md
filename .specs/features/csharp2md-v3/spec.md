...
| FACT-55 | P3: Migration and release | T41's `MigrationLedgerTests` runs all 428 baseline rows as a category-level check (12 namespace-family representatives, corrected 2026-08-19 per the independent Verifier's finding) and proves every named v2 production path is absent from the assembly; the actual per-behavior replacement is the ~730 v3-era tests added across T1-T47 | Verified |
