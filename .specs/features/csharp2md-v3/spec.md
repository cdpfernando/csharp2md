...
| FACT-46 | P2: Components and relations | T37 confirmed gRPC relation detector implemented (namespace-qualified generated-client base type, no name-only matching) | Implementing |
| FACT-47 | P2: Components and relations | T36-T37 (HTTP, gRPC) relations always leave `TargetId: null` with `UnresolvedReason` set; T38 applies the same rule to messaging | Implementing |
...
| FACT-58 | P3: Migration and release | T34-T37 (ASP.NET Core, dependency-injection, HTTP, gRPC) carry 83 combined positive/negative/lookalike spec-derived cases; T38-T39 close remaining detector families | Implementing |
