...
- [ ] `python3 .claude/skills/tlc-spec-driven/scripts/validate_state.py generator-cli-projections-certification` exits clean — **stays unchecked, permanently, by design.** The feature-level Verifier ran three fix→re-verify iterations (the skill's bound); iteration 3's `validation.md` verdict is FAIL, left true and unedited (evidence integrity — the Verifier's own report is never rewritten to a false PASS). Per AD-028 in `.specs/STATE.md`, the user chose to close this feature now with two Major gaps (GCPC-012/016/018/034/088) plus a minor and a cosmetic one deferred to a future workstream, rather than take a 4th round. `validate_state.py` will therefore keep exiting 1 for this feature forever — that is the accepted trade-off, not an open task.
...
- [x] Full suite reported green with its final total — confirmed by the iteration-3 Verifier (final): 2072 total, 0 failed, `Category=LocalCorpus` excluded (Domain 563, Analysis 827, Storage 389, Cli 64, Projection 229). "Green" here means the test suite, not the Verifier's verdict — the verdict is FAIL for the reasons above, recorded as AD-028, and that is what this box's sibling above stays unchecked for.
