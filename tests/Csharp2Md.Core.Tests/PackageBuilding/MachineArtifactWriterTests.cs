...
public sealed class MachineArtifactWriterTests
{
    ...
    [Fact][Trait("Requirement", "PKG-01")] public void Write_ListsProvenRoot() => Assert.Equal("component:orders", Assert.Single(Roots(Write()).Roots).DisplayName);
    [Fact][Trait("Requirement", "STO-05")] public void Write_RootCitesMachineArtifactByHandle() { var index = Roots(Write()); Assert.Matches("graph/entities.000000.json#[0-9a-z]{1,6}$", index.MachineCitation(Assert.Single(index.Roots).Handle)); }
    ...
    private static RootsIndexData Roots(MachineArtifactSet machine) =>
        CanonicalJson.Read<RootsIndexData>(machine.Artifacts.Single(artifact => artifact.Path.Value == Assert.Single(machine.Manifest.Solutions).Roots.EntryPath).Payload.AsSpan());
