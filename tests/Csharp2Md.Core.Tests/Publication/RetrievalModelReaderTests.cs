...
public sealed class RetrievalModelReaderTests
{
    ...
    [Fact][Trait("Requirement", "EDG-05")] public void VerifyMarkdown_RejectsMutatedMarkdownWithPath() { var artifacts=Artifacts(); var path=RootPage(artifacts); artifacts[path]=Encoding.UTF8.GetBytes("changed\n").ToImmutableArray(); Assert.Equal(path, Assert.Throws<PackageCorruptionException>(() => RetrievalModelReader.VerifyMarkdown(artifacts)).Artifact); }
    [Fact][Trait("Requirement", "EDG-05")] public void VerifyMarkdown_RejectsMissingMarkdownWithPath() { var artifacts=Artifacts(); var path=RootPage(artifacts); artifacts.Remove(path); Assert.Equal(path, Assert.Throws<PackageCorruptionException>(() => RetrievalModelReader.VerifyMarkdown(artifacts)).Artifact); }
    [Fact][Trait("Requirement", "PUB-03")] public void Read_RejectsATamperedDocumentsIndex() { var artifacts=Artifacts(); var path=DocumentsIndexPath(artifacts); artifacts[path]=CanonicalJson.Write(new DocumentsIndexData("solutions/tampered/markdown/documents/", [])); Assert.Equal(path, Assert.Throws<PackageCorruptionException>(() => RetrievalModelReader.Read(artifacts)).Artifact); }
    [Fact][Trait("Requirement", "PUB-03")] public void Read_RejectsAMissingDocumentsIndex() { var artifacts=Artifacts(); var path=DocumentsIndexPath(artifacts); artifacts.Remove(path); Assert.Equal(path, Assert.Throws<PackageCorruptionException>(() => RetrievalModelReader.Read(artifacts)).Artifact); }
    private static string DocumentsIndexPath(IReadOnlyDictionary<string, ImmutableArray<byte>> artifacts) => CanonicalJson.Read<RootsIndexData>(artifacts[Assert.Single(Manifest().Solutions).Roots.EntryPath].AsSpan()).DocumentsIndexPath;
    private static string RootPage(IReadOnlyDictionary<string, ImmutableArray<byte>> artifacts) { var declared=Assert.Single(Manifest().Solutions).Roots; var index=CanonicalJson.Read<RootsIndexData>(artifacts[declared.EntryPath].AsSpan()); return index.MarkdownPath(index.Roots[0].Handle); }
