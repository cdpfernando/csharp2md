...
public sealed class JourneyCertifierTests
{
    ...
    [Fact][Trait("Requirement", "CRT-02")] public void Certify_EmptyRootsIsNotApplicable() { using var package = Package(roots: []); var journey = Journeys(package).Single(x => x.Kind == JourneyKind.Locate); Assert.Equal(JourneyCertificationStatus.NotApplicable, journey.Status); Assert.StartsWith("not_applicable:", journey.Detail); }
    [Fact][Trait("Requirement", "CRT-02")] public void Certify_GraphJourneysAreNotApplicableUntilTheirCertifierRuns() { using var package = Package(); Assert.All(Journeys(package).Where(x => x.Kind is JourneyKind.FollowFlow or JourneyKind.ReverseImpact), x => Assert.Equal(JourneyCertificationStatus.NotApplicable, x.Status)); }
    ...
    [Fact][Trait("Requirement", "CRT-01")] public void Certify_MissingEvidenceTerminalFails() { using var package = Package(); using var reader = PackageReader.Open(package.Path); var solution = Assert.Single(reader.Manifest.Solutions); var evidence = JourneyCertifier.Entry(solution, JourneyKind.EvidenceDisposition); File.Delete(Path.Combine(package.Path, evidence.Replace('/', Path.DirectorySeparatorChar))); Assert.Equal(JourneyCertificationStatus.Failed, Evidence(package).Status); }
    ...
    private static ImmutableArray<JourneyCertification> Journeys(TempPackage package) =>
        Assert.Single(JourneyCertifier.Certify(package.Path).Solutions).Journeys;

    private static JourneyCertification Locate(TempPackage package) => Journeys(package).Single(x => x.Kind == JourneyKind.Locate);
    private static JourneyCertification Evidence(TempPackage package) => Journeys(package).Single(x => x.Kind == JourneyKind.EvidenceDisposition);
