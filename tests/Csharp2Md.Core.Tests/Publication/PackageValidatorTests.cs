...
public sealed class PackageValidatorTests
{
    ...
    [Fact][Trait("Requirement", "EDG-05")] public void Validate_MutatedMarkdownReportsArtifact() { using var package = Package(); var path = Assert.Single(Open(package).Manifest.Solutions).Roots[0].MarkdownPath; File.WriteAllText(Path.Combine(package.Path, path.Replace('/', Path.DirectorySeparatorChar)), "changed\n"); Assert.Equal(path, Assert.Single(Validate(package).Failures).Artifact); }
