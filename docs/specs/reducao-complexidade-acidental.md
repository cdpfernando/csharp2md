...
**Revisão:** 2026-09-14, sobre HEAD `0c3999e` e alterações locais. A revisão de 2026-09-11 (HEAD `2e93f5f`) permanece a origem dos 13 itens; esta edição reconcilia o estado observado depois disso.
...
1. **Documentação e regras ativas:** concentrar o estado de implementação em sua fonte canônica. READMEs, o índice de arquitetura, a arquitetura principal e as instruções devem apontar para esse estado e abandonar afirmações de que a substituição inteira ainda não foi implementada, de que faltam recuperação, composição e certificação, ou de que `retrieval-projections` ainda é a spec em curso. Preservar AD-028 e suas lacunas explicitamente adiadas.
...
A base observada nesta edição é o HEAD `0c3999eaf31ef2ce53f1c4e9886fa37083b9066d` (codesync 2026-09-14) acrescido das alterações locais ainda não commitadas. A revisão original usava `2e93f5f` (fechamento AD-028) mais o trabalho local de então; entre os dois HEADs o detalhe sanitizado de falha do pipeline entrou no histórico (`PipelineFailureDetail` e testes associados). Este documento consolida a revisão no formato da skill to-spec e permanece local. Sua edição e classificação não iniciam a implementação.

Estado dos itens: implementação observada dos itens 2 e das histórias 7–8; atendimento parcial dos itens 1, 3, 6 e 8; itens 4, 5, 7 e 9 a 13 abertos; itens 14–16 abertos (achados de 14/09). Observação estática não equivale a nova validação funcional. A issue local `01-expose-safe-pipeline-failure-details` descreve trabalho já presente no HEAD; não tratá-la como entrega aberta.
...
**Item 1 — Concentrar regras e eliminar instruções obsoletas (parcial).** Os dois READMEs e [architecture-knowledge-engine.md](../architecture/architecture-knowledge-engine.md) ainda afirmam que a substituição não foi implementada. [docs/architecture/README.md](../architecture/README.md) está mais atrasado: declara workstreams 1–5D no `master`, diz que faltam recuperação, composição e certificação, e aponta `retrieval-projections` como spec em curso. AGENTS.md e CLAUDE.md repetem que ainda não existe spec de substituição; isso é a regra de não criar feature nova fora do roadmap, não a negação das oito features encerradas. Estado e roadmap já registram workstreams 1–8 encerrados com lacunas adiadas (AD-028). A divergência pode induzir a recriação de trabalho entregue.

Referências: [AGENTS.md](../../AGENTS.md), [README](../../README.md), [README em português](../../README.pt-BR.md), [índice de arquitetura](../architecture/README.md), [arquitetura](../architecture/architecture-knowledge-engine.md), [estado](../../.specs/STATE.md), [roadmap](../../architecture-knowledge-engine-roadmap.md).
