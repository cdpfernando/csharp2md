...
- The only versioned analysis fixture is `fixtures/SyntheticSolution`. Never commit `fixtures/eShop` or `fixtures/eShopOnContainers` (gitignored local clones). After each feature Verifier, if those clones are present, run the `LocalCorpus` tests or the matching CLI launch profiles; skip when they are absent. Do not fail CI for a missing local clone.
