...
| P0 | Índice de recuperação compacto | **IMPLEMENTADO — AGUARDANDO VERIFIER** | `compact-retrieval-index` | `relation-retrieval-index` | Cada relação derivada é serializada uma vez; consultas usam postings compactos; shards deixam de reserializar prefixos e de criar um diretório por chave; só muda para `CONCLUÍDO` após PASS independente |
