...
using SistemaE.Formalizacao;
using SistemaE.Identificacao;
using SistemaE.Infraestrutura;
using SistemaE.Oferta;
using SistemaE.PosProcessamento;
using SistemaE.Validacao;
using Xunit;

namespace SistemaE.Testes;
...
public class VendasTests
{
    ...
    public async Task ProcessarVenda_DeveExecutarTodosOsModulosGravarNoBancoEPublicarEvento()
    {
        ...
        var eventoPublicado = Assert.IsType<SistemaE.Nucleo.VendaConcluida>(fakeBus.Events[0]);
    ...
    public async Task PedidoRecebidoIntegrationEventHandler_DeveExecutarFluxoDeVendaComSucesso()
    {
        ...
        Assert.Single(fakeBus.Events);
        Assert.IsType<SistemaE.Nucleo.VendaConcluida>(fakeBus.Events[0]);
    }
}
