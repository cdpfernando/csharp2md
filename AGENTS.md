...
- `CONTEXT.md` defines the product language, `.specs/STATE.md` owns active decisions and execution state, and `docs/specs/pacote-conhecimento-util-e-confiavel.md` defines the proposed next product contract. Existing source and schemas are the operational baseline, not a source for the next replacement's semantics.
- Spec-driven work lives in `.specs/features/`. The next replacement exists only as a proposal in `docs/specs/pacote-conhecimento-util-e-confiavel.md`; create its executable feature only when the user explicitly starts that workstream. Execute it through `.agents/skills/tlc-spec-driven/SKILL.md` and its referenced files/scripts.
- Do not load or recreate superseded specs, tickets, ADR prose, relation taxonomies, CLI contracts or output schemas from Git history unless the user explicitly requests historical analysis. Completed feature artifacts are historical evidence for their own deliveries, not the source of future product semantics.
...
- `tlc-spec-driven`'s discrimination sensor (the mutation-testing step inside Execute/Validate) is a standing **skip** for this project, overriding the skill's own "never optional, never prompted" default. The user runs Stryker manually and does not want the sensor's fault-injection pass run by the agent. All completed features have skipped it; do not re-ask or re-decide per feature — note the skip in each new feature's `tasks.md` header (same phrasing as the prior features) and move on. Every other Verifier step (spec-anchored coverage check, gate check, code-quality check) still runs as documented.
...
- The versioned analysis fixtures are `fixtures/SyntheticSolution`, `fixtures/CertificationCorpus`, and `fixtures/PublicationResilience`. `CertificationCorpus` is the labeled engine-certification tree; `PublicationResilience` is an unlabeled publication-regression tree. Never commit `fixtures/eShop` or `fixtures/eShopOnContainers` (gitignored local clones). After each feature Verifier, if those clones are present, run the `LocalCorpus` tests or the matching CLI launch profiles; skip when they are absent. Do not fail CI for a missing local clone.
